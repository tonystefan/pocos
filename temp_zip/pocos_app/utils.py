...
def gerar_datas_aleatorias(ano_inicio, ano_fim, meses_selecionados, limitar_data_atual=True):
    """
    Gera todas as datas do período, marcando quais meses estão selecionados.
    
    Args:
        ano_inicio: Ano inicial
        ano_fim: Ano final
        meses_selecionados: Lista de códigos de meses selecionados (ex: ["jan", "fev"])
        limitar_data_atual: Se True, limita até o dia anterior à data atual
        
    Returns:
        Lista de tuplas (data, mes_selecionado) ordenadas
    """
    datas = []
    ...
    # Obter data limite se necessário
    data_limite = None
    if limitar_data_atual:
        # Limitar até o dia anterior à data atual
    ...
    for ano in range(ano_inicio, ano_fim + 1):
        ...
        for codigo_mes, num_mes in meses_map.items():
            ...
            mes_selecionado = codigo_mes in meses_selecionados
            ...
            # Incluir todos os dias do mês, respeitando a data limite
            for dia in range(1, dias_no_mes + 1):
                try:
                    data_atual = datetime(ano, num_mes, dia)
                    
                    # Verificar se deve limitar pela data atual
                    if data_limite and data_atual.date() > data_limite:
                        # Se passou da data limite, parar de adicionar datas
                        return datas
                    
                    datas.append((data_atual, mes_selecionado))
                except ValueError:
...
def gerar_tabela_dados(parametros):
    ...
    limitar_data_atual = parametros.get("limitar_data_atual", True) # Obter opção de limitação

    # Gerar todas as datas do período com informação se o mês está selecionado
    datas_info = gerar_datas_aleatorias(ano_inicio, ano_fim, meses_selecionados, limitar_data_atual)
    if not datas_info:
    ...
    datas_str = [info[0].strftime("%d/%m/%Y") for info in datas_info]
    num_datas = len(datas_info)
    
    # Mapeamento reverso de meses
    ...
    for i, (data_dt, mes_selecionado) in enumerate(datas_info):
        ...
        # Calcular Volume diário (m3) e Tempo diário
